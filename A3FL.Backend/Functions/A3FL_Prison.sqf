...
["A3PL_Prison_LockpickCell",
{
	...
	if(Player_ActionInterrupted) exitWith {["Failed to lockpick cell door!", "red"] call A3PL_Player_Notification;};
...
["A3PL_Prison_DigOut",
{
	...
	if (_factionReq) exitwith {["There needs to be 3 FIMS on-duty to dig out of jail!","red"] call A3PL_Player_Notification;};
	if (player getVariable ["Digging",false]) exitWith{[localize"STR_NewRessources_Action","red"] call A3PL_Player_Notification;};
	...
	["Digging out of jail...",30] spawn A3PL_Lib_LoadAction;
	waitUntil{Player_ActionDoing};
	while {Player_ActionDoing} do {
	...
	if(Player_ActionInterrupted) exitWith {[localize"STR_NewRessources_DiggingCanceled","red"] call A3PL_Player_Notification;};
